...
var phonographEvents = phonographEvents || \[\]; (function() { var continent = document.cookie.match(/\_chorus\_geoip\_continent=(\[^;\]+)/i) || \[\]; var data = { 'Page Title':document.title, 'GA Track Prefix':'t1.', 'geoip\_continent': continent\[1\] }; var dataLayer = window.dataLayer || \[\]; dataLayer .filter(function(m) { return !!m\['Network'\] }) .forEach(function(m) { Object.keys(m).forEach(function(k) { data\[k\] = m\[k\] }); }); var userid = data\['Logged in Status'\]; data\['User ID'\] = (typeof userid === 'number') ? userid : null; phonographEvents.push(\['pageload', data\]); })();